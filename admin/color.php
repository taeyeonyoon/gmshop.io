...
function Setting(color)
{
	...
	opener.document.copyForm.copyBC.value =color;<?
		}
		else
		{
			?>
	opener.document.getElementById('<?=$target?>').style.color =color;
	opener.document.copyForm.copyTC.value =color;<?
		}
	}
	else
	{
		if($part2=="bg")
		{
			?>
	opener.document.getElementById('<?=$target?>').bgColor =color;
	var colorTextBox = eval("opener.document.getElementById('t_<?=$target?>')" );
	colorTextBox.value = color;
	var colorText = eval("opener.document.getElementById('text_<?=$target?>')" );
		...
		<td height="70">
			<table width="300" border="0" cellspacing="0" cellpadding="0" height="70" background="image/color_bg.gif">
				...
				for($i=0;$i<18;$i++)
				{
					...
					?>
					<td><div align="center"><a href="color.php?colorIndex=<?=$i?>&target=<?=$target?>&part1=<?=$part1?>&part2=<?=$part2?>"><img src="image/color_<?=$img_num?>.gif" width="21" height="21" border="0"></a></div></td><?
					if($i==8)
					{
						?>
				</tr>
