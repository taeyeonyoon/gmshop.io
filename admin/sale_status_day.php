...
$PrevDay = date("Ymd",mktime(0,0,0,$smonth,$sday-1,$syear));
$NextDay = date("Ymd",mktime(0,0,0,$smonth,$sday+1,$syear));
...
$CurMonth = _get_sale($syear,$smonth,$sday);
?>
<style>
		...
		<td width="85%" valign="top">
			<table width="100%" border="0" cellspacing="0" cellpadding="0" align='center'>
				...
				<tr>
					<td>
						<table width="750" border="0" cellspacing="0" cellpadding="0" align="center">
							...
							<tr>
								<td width='440'><img src='image/sale_tit3.gif'></td>
							</tr>
							<tr>
								<td width='1' bgcolor='dadada' colspan='3'></td>
							</tr>
				...
				<tr>
					...
					for ($i=1; $i<32; $i++)
					{
						$sday_sel[$sday]="selected";
						?><option value="<?=sprintf("%02d",$i)?>" <?= $sday_sel[sprintf("%02d",$i)]?>><?=sprintf("%02d",$i)?>일</option><?
					}
					?></select> <input type='image' src='image/view_btn.gif' border='0' align='absmiddle'>&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?= $_SERVER[PHP_SELF]?>?syear=<?=substr($PrevDay,0,4)?>&smonth=<?=substr($PrevDay,4,2)?>&sday=<?=substr($PrevDay,6,2)?>"><img src='image/pre_day.gif' border='0' align='absmiddle'></a>&nbsp;&nbsp;<a href="<?= $_SERVER[PHP_SELF]?>?syear=<?=substr($NextDay,0,4)?>&smonth=<?=substr($NextDay,4,2)?>&sday=<?=substr($NextDay,6,2)?>"><img src='image/next_day.gif' border='0' align='absmiddle'></a></td>
				</tr>
				</form>
				<tr>
					<td align='center'><br>
						...
						<table width='750' border='0' cellspacing='1' cellpadding='2' bgcolor='#C7C7C7'>
							...
							<tr bgcolor='#FFFFFF'>
								...
								<td> <font class='money_class'><?= number_format($CurMonth[goods][$row[idx]][a])?> 원</font></td>
									...
									$goods_a += $CurMonth[goods][$row[idx]][a];
								}
							}
							?>
							<tr bgcolor='#FFF2E5'>
								<td align='center' style='padding:2 0 2 5'><b>합계</b></td>
								<td style='padding:2 0 2 5'><font class='ea_class'><?= number_format($CurMonth[ea][a])?></font></td>
								<td><font class='money_class'><?= number_format($goods_a)?> 원</font></td>
							</tr>
						</table><br><br>
						<a name="03" id="03"></a>
						<table width='750' border='0' cellspacing='1' cellpadding='2'>
