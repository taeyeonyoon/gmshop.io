							...
							<tr>
								<td valign="top">
									<table width="750" border="0" cellspacing="1" cellpadding="2" bgcolor='cdcdcd'>
										<tr valign="middle">
											<td width="80" bgcolor="#EBEBEB" height="30"> <div align="center"><img src="image/icon.gif" width="11" height="11"> 월</div></td>
										...
										for($i=0,$mon=1;$i<12;$i++,$mon++)
										{
											$qry = "select sum(cnt),sum(price) from trade_goods where month(sday1) =$mon and status>0 and status<4 ";
											$result = $MySQL->query($qry);
											$row = mysql_fetch_array($result);
											if($total_buyNum[0])			  $cntP			= $row[0]/$total_buyNum[0]*100*2.3;					//구매수 퍼센트
											else				  	  $cntP			= 0;			//구매수 퍼센트
											$sumPrice		= $MySQL->fetch_array("select sum(payM) from trade where month(writeday)=$mon and bPay=1 and status>0 and status<4");
											if($total_buyMoney[0])		  $priceP		= $sumPrice[0]/$total_buyMoney[0]*100*2.3;			//구매액 퍼센트
											else					  $priceP		= 0;
											if(empty($row[0])) $row[0]=0;
											?>
										<tr>
											<td width="80" height="30" bgcolor="ffffff"> <div align="center"><?=$mon?>월</div></td>
											...
											if ($sumPrice[0])
											{
												?>
										<tr bgcolor="ffffff">
											<td valign="middle" align="center"><?=$mon?>월<br>우수구매자</td>
											<td colspan="3">
												<table class="table_coll" border=1 width="80%" align="center">
													...
													$best_result  = $MySQL->query("SELECT userid,sum(payM),name,userid_part from trade WHERE month(sday1) = $mon and status>0 and status<4 group by userid order by 2 desc limit 5");
													$cnt=1;
